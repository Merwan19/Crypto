...
/*
    fichier1 = fopen("file.txt", "r+");
    if (fichier1 != NULL)
    {
	while(fgets(ligne,128,fichier1)!=NULL){
	printf("ligne lu : %s",ligne);
AES_set_encrypt_key(K, 16, &wctx);
AES_encrypt(ligne, ligneEnc, &wctx);
	printf("ligne enc : %s\n",base64encode(ligneEnc,16));
AES_decrypt(ligneEnc, ligneDec , &wctx);
	printf("ligne dec : %s\n",ligne);
	}
...
void decrypt(FILE *ifp, FILE *ofp,char* ckey)
{
    ...
    //setup decryption
    EVP_CIPHER_CTX ctx;
    EVP_DecryptInit(&ctx,EVP_aes_128_cbc(),ckey,NULL);
...
void encrypt(FILE *ifp, FILE *ofp,char* ckey)
{
    ...
    //Set up encryption
    EVP_CIPHER_CTX ctx;
    EVP_EncryptInit(&ctx,EVP_aes_128_cbc(),ckey,NULL);
